...
class HurstExponent(PeriodN):
   ...
   Interpretation of the results
    ...
    def __init__(self):
        ...
        self._lag_start = lag_start = self.params.lag_start or 2
        self._lag_end = lag_end = self.params.lag_end or (self.params.period // 2)
        self.lags = np.asarray(range(lag_start, lag_end))
        self.log10lags = np.log10(self.lags)

    def next(self):
        # Fetch the data
        ts = np.asarray(self.data.get(size=self.params.period))

        # Calculate the array of the variances of the lagged differences
        tau = [np.sqrt(np.std(np.subtract(ts[lag:], ts[:-lag]))) for lag in self.lags]

        # Use a linear fit to estimate the Hurst Exponent
        poly = np.polyfit(self.log10lags, np.log10(tau), 1)

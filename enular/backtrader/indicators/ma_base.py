...
class MovingAverage(object):
    ...
    """MovingAverage (alias MovAv) - a placeholder to gather all moving average
    types in a single place.

    Instantiating a moving average can be achieved as follows:

        - sma = MovingAverage.Simple(self.data, period)

    Or using shorter aliases:

        - sma = MovAv.SMA(self.data, period)

    Or with full backwards and forwards names:

...
class ParabolicSAR(PeriodN):
    ...
    """Parabolic Stop and Reverse - used to determine the price direction of an
    asset and when direction is beginning to change.
    ...
    parameters above. Examples:
        ...
        - ParabolicSAR(self.data, period=self.params.period)
        - ParabolicSAR(self.data, period=self.p.period, af=self.p.af)
        - ParabolicSAR(self.data, period=2, af=0.02, afmax=0.2)
        - ParabolicSAR(self.data)
    ...
    plotlines = dict(psar=dict(marker='.',
                               markersize=4.0,
                               color='navy',
                               fillstyle='full',
                               ls=''))
    ...
    def next(self):
        ...
        if (tr and sar >= lo) or (not tr and sar <= hi):
            ...
            af = self.params.af  # reset acceleration factor
        ...
        if tr:  # long trade
            if hi > ep:
                ep = hi
                af = min(af + self.params.af, self.params.afmax)
